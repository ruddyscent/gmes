...
explicit finite-difference time-domain (FDTD) simulation. The supported solver
API is implemented in `gmes/`; examples live in `examples/`, and component
tests live in `tests/`. Historical native sources and measurements are not
active build or runtime dependencies.
...
- Use the declarative PEP 517 build declared in `pyproject.toml`; it must not
  require a GMES compiler, SWIG, Cython, OpenMP runtime, or MPI.
...
the repository root. Select one Torch runtime extra; CPU is the normal local
and CI choice.
...
- Do not commit generated build artifacts or simulation results.
...
- Verify that the labels were applied after creation; do not leave new issues
  or pull requests unlabeled.
- Record insights discovered while working on an issue as comments on that
  issue. Include useful findings such as root causes, experimental results,
  constraints, rejected approaches, and follow-up work, with supporting code
  references or verification evidence where available.
- Keep insight comments concise, distinguish verified findings from
  hypotheses, and avoid repeating existing comments. Record findings as they
  become actionable and summarize any remaining insights before closing the
  issue.
