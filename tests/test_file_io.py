...
class TestProbe:
    def test_probe_writes_metadata_and_sample(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with closing(Probe((1, 0, 1), field, output)) as probe:
                probe.write_header((0.5, 0.0, -0.5), 0.25)
                probe.write(3)

            assert output.read_text() == "# location=(0.5, 0.0, -0.5)\n# dt=0.25\n3 5\n"

    @pytest.mark.skipif(
        not (importlib.util.find_spec("tables")), reason="PyTables is not installed"
    )
