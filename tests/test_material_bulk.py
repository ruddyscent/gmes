...
class TestMaterialMappingFastPath:
    """Keep vectorized lowering equivalent to pointwise geometry semantics."""

    def _assert_planner_maps_match_pointwise_geometry(self, simulation, name):
            ...
            for y in axes[1]:
                for z in axes[2]:
                    ...
                    expected_underlying.append(
                        -1 if underneath is None else geometry_ids[id(underneath)]
                    )
    ...
    @pytest.mark.parametrize(
        "size", ((1, 1, 1), (2, 0, 0)), ids=("three-dimensional", "one-dimensional")
    )
    @pytest.mark.parametrize(
        "name", tuple(_COMPONENT_TYPES), ids=tuple(_COMPONENT_TYPES)
    )
    def test_batched_mapping_matches_pointwise_for_all_components_and_clipping(
        self, size, name
    ):
        ...
        assert top is simulation.geometry[3]
        assert underneath is simulation.geometry[2]
        self._assert_planner_maps_match_pointwise_geometry(simulation, name)
        assert np.any(simulation.plan.components["Ex"].underlying_ids == 2)
    ...
    def test_pointwise_and_vectorized_geometry_have_equal_multistep_fields(self):
        ...
        assert not GeomBoxTree(pointwise.geometry).supports_bulk_lowering()
        assert PointwiseSphere.calls > 0
        for _ in range(3):
            vectorized.step()
            pointwise.step()
    ...
    def test_opted_in_custom_geometry_uses_vectorized_lowering(self):
        ...
        assert GeomBoxTree(opted_in.geometry).supports_bulk_lowering()
    ...
    def test_custom_material_subclasses_fail_closed_before_runtime_execution(self):
        ...
        with pytest.raises(NotImplementedError, match="does not support material"):
