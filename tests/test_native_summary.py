...
class NativeSummaryTest(unittest.TestCase):
    ...
    def _environment(self, threads, scaling):
        return {
            ...
            "git_commit": self.reference["performance_observer_commit"],
            "git_status": "",
            "uv_lock_sha256": "a" * 64,
    ...
    def _write_gpu_topology_inputs(self, directory, first_topology):
        paths = self._write_inputs(directory)
        for index, path in enumerate(paths):
            cell = json.loads(path.read_text())
            cell["environment"]["gpu_topology"] = (
                first_topology if index == 0 else "GPU0 GPU-0000"
            )
            path.write_text(json.dumps(cell, indent=2, sort_keys=True) + "\n")
        return paths
    ...
    def test_normalizes_only_balanced_gpu_topology_underline_sgr(self):
        with tempfile.TemporaryDirectory() as directory:
            paths = self._write_gpu_topology_inputs(
                directory,
                "\x1b[4mGPU0\x1b[0m GPU-0000",
            )
