...
class TestGeometryMapCompatibility:
    @pytest.fixture(autouse=True)
    def _cartesian_space(self):
    ...
    def test_region_ids_preserve_overlap_and_immediate_underlying_region(self):
        ...
        assert geometry_map.materials == tuple(obj.material for obj in geometry)
        assert geometry_map.shape == (4, 1, 1)
        assert (geometry_map.start, geometry_map.stop) == (0, 4)
        assert geometry_map.component == "Ez"

    @pytest.mark.parametrize(
        "case_index",
        range(6),
        ids=("sphere", "block", "ellipsoid", "cylinder", "cone", "shell"),
    )
    def test_boundary_and_adjacent_points_match_scalar_lookup(self, case_index):
        cases = (
        ...
        geometry, boundary = cases[case_index]
...
class TestGeometryPickleCompatibility:
    @pytest.mark.parametrize(
        ("state", "payload"),
        LEGACY_CYTHON_SPHERE_PICKLES.items(),
        ids=LEGACY_CYTHON_SPHERE_PICKLES,
    )
    def test_loads_cython_sphere_pickles_before_and_after_initialization(
        self, state, payload
    ):
        sphere = pickle.loads(base64.b64decode(payload))
        assert isinstance(sphere, Sphere)
        assert sphere.material.eps_inf == 2.5
        assert sphere.radius == 0.4
        np.testing.assert_array_equal(sphere.center, (0.25, -0.5, 0.75))
        if state == "before_init":
            assert sphere.box is None
        else:
    ...
    def test_current_geometry_pickle_and_deepcopy_preserve_point_queries(self):
        ...
        for restored in (pickle.loads(pickle.dumps(tree)), deepcopy(tree)):
            assert [type(restored.material_of_point(point)[0]) for point in points] == [
