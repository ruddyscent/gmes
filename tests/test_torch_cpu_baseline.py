...
class TorchCpuBaselineTest(unittest.TestCase):
    ...
    def test_loads_two_slices_and_records_exact_source_hashes(self):
        ...
        self.assertEqual(
            {item["path"]: item["sha256"] for item in baseline["source_artifacts"]},
            actual,
        )
        ...
        self.assertEqual(baseline["environment"]["cpu_model"], "baseline CPU")
        self.assertIn(
            "CPU(s) scaling MHz:", baseline["source_artifacts"][0]["raw_cpu_model"]
        )
    ...
    def test_rejects_incomplete_or_different_host_identity(self):
        one = self._artifact(1)
        del one["environment"]["cpu_model"]
        with self.assertRaisesRegex(ValueError, "host identity is incomplete"):
            self._load(one=one)
    ...
    def test_does_not_trust_legacy_allocation_or_pass_flags(self):
        one = self._artifact(1)
        one["cases"][0]["acceptance"]["recurring_allocations_zero"] = True
        one["cases"][0]["acceptance"]["passed"] = True
        baseline, _paths = self._load(one=one)
        self.assertEqual(len(baseline["source_artifacts"]), 2)
