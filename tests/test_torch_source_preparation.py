...
class TestTorchSourcePreparation:
    @pytest.mark.parametrize(
        ...
        (
            ...
            "nan amplitude",
            "infinite amplitude",
            "negative infinite amplitude",
            "nan current",
            "infinite current",
            "positive infinite current",
        ),
        ids=lambda value: value.replace(" ", "-"),
    )
    def test_invalid_sources_fail_before_plan_or_state_allocation(self, invalid_name):
    ...
    def test_exact_extension_lowers_once_with_frozen_context_and_infinite_end(self):
        ...
        assert math.isinf(waveform.end)
        ...
        assert tuple(field.name for field in fields(type(source.context))) == (
            "space",
            "geometry_tree",
            "paired_real",
            "dtype",
        ...
        assert source.context.space is simulation.space
        assert source.context.geometry_tree is simulation.geom_tree
        assert source.context.paired_real == simulation.state.paired_real
        assert source.context.dtype == simulation.dtype
        assert source.context.device == simulation.device
        assert source.context.dt == simulation.plan.dt
    ...
    def test_point_and_current_sources_apply_expected_local_values(self):
        ...
        simulation.step()
        assert (
            round(abs(float(simulation.state.ex[1, 1, 1]) - 2.0 * expected_waveform), 7)
            == 0
        )
        assert (
            round(
        ...
        assert shared_batch.overwrite_targets.numel() == 0
        assert shared_batch.additive_targets.numel() == 1
